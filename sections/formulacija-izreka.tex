...
\begin{document}
Naj bo $I\subseteq \R$ povezana podmnožica realnih števil. Take množice bomo imenovali intervali. Interval ni nujno zaprt ali omejen in lahko v nekaterih primerih predstavlja kar celotno množico realnih števil. Naj bo $f \colon I \to I$ zvezna funkcija, ki slika interval $I$ nazaj vase. Za naravno število $n$ bomo s $f^n$ označevali kompozitum
...
\begin{figure}[h]
  \centering
  \includegraphics[]{iteracije_f.pdf}
...
\begin{definicija}
Naj bo $M$ množica. Relacija $\leq$ definirana na množici $M$ je relacija delne urejenosti, če veljajo naslednje lastnosti:
\begin{itemize}
\item \emph{refleksivnost}: $\forall a \in M : a \leq a$,
\item \emph{antisimetričnost}:  $\forall a, b \in : a \leq b \text{ in } b \leq a \Rightarrow a = b$,
\item \emph{tranzitivnost}: $\forall a, b, c \in M : a \leq b \text{ in } b \leq c \Rightarrow a \leq c$.
\end{itemize}
Relacija $\leq$ je linearna urejenost, če je relacija delne urejenosti, ki je \emph{sovisna}. To pomeni, da sta vsaka dva elementa v relaciji $\leq$. Natančneje: za vsaka elementa $a, b \in M$ velja $a \leq b$ ali $b \leq a$.
Stroga linearna urejenost je relacija $<$, ki je tranzitivna, sovisna in \emph{irefleksivna}. Irefleksivnot pomeni, da ne obstaja element $a \in M$, za katerega je $a<a$.
\end{definicija}
...
\begin{opomba}
Hitro lahko vidimo, da je relacija $\triangleright$, ki smo jo definirali, relacija linearne urejenosti. Očitno je relacija refleksivna. Ker se ne more zgoditi, da v ureditvi Šarkovskega število $a$ leži levo od števila $b$ in hkrati število $b$ leži levo od števila $a$, sta lahko pogoja $a \triangleleft b$ in $b \triangleleft a$ izpolnjena samo v primeru, ko je $a = b$. Zato je relacija antisimetrična. Če število $a$ leži levo od števila $b$ v ureditvi Šarkovskega in število $b$ leži levo od števila $c$ v ureditvi Šarkovskega, potem je jasno, da v ureditvi Šarkovskega število $a$ leži levo od števila $c$, kar pomeni, da je izpolnjena relacija $a \triangleleft c$ in je s tem relacija $\triangleleft$ tranzitivna. Ker v ureditvi Šarkovskega nastopajo vsa naravna števila, lahko za vsaki dve različni naravni števili ugotovimo, katero število leži levo od drugega števila, zato je relavija sovisna.
\end{opomba}

\iffalse % Začetek dokaza s pomočjo drugačnega zapisa števil sem skril, ker sva z mentorjem ocenila, da ga verjetno ne bom potreboval. Če se izkaže, da bi bilo dobro izvesti dokaz lahko zadevo zopet prikažem
\begin{proof}
...
\end{proof}
\fi %konec dokaza, ki sem ga skril ########################
...
Sedaj smo definirali vse potrebne pojme in spoznali tudi ureditev Šarkovskega, v nadaljevanju pa si bomo ogledali, kako ureditev Šarkovskega določa periode funkcije.

\begin{izrek}[Izrek Šarkovskega o prisili]\label{izr:forcing}
Če ima zvezna funkcija $f \colon I \to I$ točko periode $m$ in velja $ m \triangleright l$, potem obstaja tudi točka periode $l$.
\end{izrek}
...
\begin{izrek}[Izrek Šarkovskega o realizaciji]\label{izr:realization}
Za vsak rep $\mathcal{T}$ v zaporedju Šarkovskega obstaja taka zvezna funkcija $f$, katere množica period je enaka $\mathcal{T}$.
\end{izrek}

Izrek Šarkovskega je unija izreka~\ref{izr:forcing} in izreka~\ref{izr:realization}. Podmnožica naravnih števil je množica period zvezne funkcije $f \colon I \to I$, če in samo če je množica rep ureditve Šarkovskega. Nasledja poglavja so namenjena pripravi in dokazu izreka~\ref{izr:forcing}, v poglavju~\ref{sec:realizacija} pa je predstavljen dokaz izreka~\ref{izr:realization}.

\section{Intervali, relacija pokritja in cikli}\label{sec:intervali}%############### Intervali, relacija pokritja in cikli
...
Naj bo $f \colon [a, b] \to [a, b]$ zvezna funkcija. Za vsako točko $y$, ki leži med točkama $f(a)$ in $f(b)$, obstaja točka $c \in (a, b)$, za katero je vrednost funkcije $f(c)$ enaka $y$.
\end{posledica}
\begin{proof}
...
Naj bosta $I, J$ zaprta intervala v množici realnih števil in naj bo $f \colon I \to \R$ zvezna funkcija. Če je $J \subseteq f(I)$, obstaja zaprt interval $K \subseteq I$, za katerega je $f(K) = J$.
\end{lema}
\begin{proof}
Interval $K$ poiščemo tako, da iz preseka grafa funkcije $f$ s pravokotnikom $I \times J$ izberemo povezan del grafa, ki povezuje spodni in zgornji del pravokotnika. Funkcija z izbiro dela grafa je prikazana na sliki~\ref{fig:bezje}. Tak del zagotovo obstaja, saj je $J \subseteq f(I)$. Projekcijo tega dela na interval $I$ označimo s $K$.
\begin{figure}[h]
  \centering
  \includegraphics[]{bezje.pdf}
% \caption[caption za v kazalo]{Dolg caption pod sliko}
...
Obravnavajmo zvezno funkcijo $f(x) = -\sqrt[3]{x}$ in 2-zanko intervalov $\left[0, \frac{1}{2}\right] \leftrightarrows \left[-\frac{1}{2}, 0\right]$. Samo ena točka sledi tej zanki, to je točka 0. Perioda točke 0 ni enaka dolžini zanke, saj je točka 0 negibna točka in je njena perioda enaka 1.
\end{primer}
...
%Vse relacije pokritja, o katerih bomo govorili, bodo $\mathcal{O}$-vsiljene, zato bodo vse relacije pokritja, ki jih bomo označili s simbolom `$\to$', predstavljale $\mathcal{O}$-vsiljene relacije pokritja. Vse relacije pokritja, o katerih bomo govorili od sedaj naprej in jih bomo označevali s simbolom `$\to$' bodo $\mathcal{O}$-vsiljene.
