...
\begin{izrek}
Za vsak rep $\mathcal{T}$ ureditve Šarkovskega obstaja zvezna funkcija $f \colon I \to I$ katere množica period je natanko množica $\mathcal{T}$.
\end{izrek}
\begin{proof}
...
\begin{split}
&T_h \colon  [0, 1] \to [0, 1], \\ 
&T_h (x) = \min \left(h, 1- 2 \left|x-\frac{1}{2} \right|\right).
\end{split}
\end{equation*}
...
Ključna ideja dokaza leme~\ref{lem:t1th} je, da definiramo funkcijo $h(\cdot)$:
\begin{equation*}
\begin{split}
&h\colon \N \to [0, 1],\\
...
S pomočjo lastnosti~\ref{l1},~\ref{l2},~\ref{l3} in izreka~\ref{izr:forcing} lahko dokažemo naslednjo lemo, s pomočjo katere zaključimo dokaz izreka Šarkovskega o realizaciji.
...
\end{proof}
Poglejmo, katere periode ima funkcija $T_{h(m)}$. Iz definicije funkcije $h(m)$ sledi, da je $m$ perioda za funkcijo $T_{h(m)}$. Radi bi videli, da ima funkcija $T_{h(m)}$ periodo $l$ natanko tedaj, ko velja $l \triangleleft m$. Najprej dokažimo implikacijo v levo smer. Naj bo $l$ naravno število, za katerega velja relacija $l \triangleleft m$. Lema~\ref{lem:<rel} zagotavlja, da velja neenakost $h(l) < h(m)$. Iz lastnosti~\ref{l1} funkcije $h$ sklepamo, da funkcija $T_{h(m)}$ vsebuje $l$-cikel. Za dokaz implikacije v desno smer predpostavimo, da funkcija $T_{h(m)}$ vsebuje $l$-cikel. Iz lastnosti~\ref{l1} funkcije $h$ sklepamo, da je $h(l) < h(m)$. Lema~\ref{lem:<rel} zagotavlja, da v tem primeru število $l$ zadošča relaciji $l \triangleleft m$. Torej je množica period za funkcijo $T_{h(m)}$ res množica $\{n \in \N; n \triangleleft m\}$.
...
Za konec pokažimo, da obstaja funkcija, ki nima periodičnih točk. Za primer lahko vzamemo translacijo $f\colon \R \to \R$ s predpisom $f(x) = x + a$, kjer je $a$ katerokoli neničelno realno število.
\end{proof}
