...
\begin{document}
V prejšnjem poglavju smo obravnavali periode zveznih preslikav definiranih na dveh disjunktnih intervalih in na krožnici. Ko smo obravnavali periode poljubne funkcije $f$ definirane na prostoru $X = I_1 \cup I_2$, smo prišli do relacije $\triangleright_X$, ki določa prisotnost period za preslikavo $f \colon X \to X$. Natančneje, če je $m$ perioda za zvezno preslikavo $f \colon X \to X$, potem je vsako naravno število $l$, za katero je $l \triangleleft_X m$, tudi perioda za preslikavo $f$. V tem poglavju bomo obravnali prostore, pri katerih je relacija, ki določa prisotnost period preslikave enaka relaciji Šarkovskega, ki smo jo spoznali v definiciji~\ref{def:ureditev-sark}. Če je relacija $\triangleright_X$ enaka relaciji Šarkovskaga, pravimo, da je prostor $X$ \emph{prostor Šarkovskega}. V tem poglavju, bomo spoznali nekaj prostorov Šarkovskega in tudi nekaj prostorov, ki to niso. S primeri in protiprimeri bomo poskušali ugotoviti katere topološke lastnosti imajo prostori Šarkovskega.
...
\end{trditev}
\begin{proof}
Naj bo prostor $X$ prostor Šarkovskega in naj bo prostor $Y$ homeomorfen prostoru $X$. Naj bo $h \colon X \to Y$ homeomorfizem med prostoroma $X$ in $Y$. Preslikavi $f \colon Y \to Y$ in $g = h^{-1} \circ f \circ h \colon X \to X$ imata enake periode, zato je $Y$ tudi prostor Šarkovskega.
\end{proof}
...
%\begin{definicija}
%Naj bo $X$ topološki prostor in $A \subseteq X$ njegov podprostor. Zvezni preslikavi $r \colon X \to A$ pravimo \emph{retrakcija}, če je preslikava $r|_A = id_A$. Podprostor $A \subseteq X$ je \emph{retrakt} prostora $X$, če obstaja retrakcija $r: X \to A$.
%\end{definicija}
...
Iz prejšnjih poglavji je razvidno, da so tipični predstavniki prostorov Šarkovskega množica realnih števil in intervali v realnih številih. V poglavju~\ref{sec:circ} smo spoznali dva prostora, ki nista prostora Šarkovskega. Največ časa smo namenili krožnici, ki ni prostor Šarkovskega, saj imajo na primer pri rotaciji za $\frac{2\pi}{3}$ vse točke periodo $3$, drugih period pa ta rotacija nima.
S podobnim sklepanjem lahko za nekatere prostore hitro preverimo, da niso prostori Šarkovskega. To naredimo tako, da poiščemo kakšno os $n$-kratne rotacijske simetrije, kjer je $n$ naravno število večje od $2$. Pri takih primerih lahko hitro ugotovimo, da ima rotacija za kot $\varphi = \frac{2\pi}{n}$, za $n>2$, točke periode $n$ in morda tudi točke periode $1$, nima pa točk periode $2$. Zaradi tega taki prostori ne morejo biti prostori Šarkovskega. Primeri takih prostorov so npr. sfera, krogla, torus \dots 
...
\end{trditev}
\begin{proof}
Naj bo prostor $X$ disjunktna unija nepraznih prostorov $A$ in $B$ in naj bosta $a \in A$ in $b \in B$ poljubni točki tega prostora. Definiramo preslikavo $f \colon X \to X$ s predpisom:
\[ f(x) = \begin{cases}
  a, & \mbox{ če $x \in B $}\\
  b ,& \mbox{ če $x \in A$.}
  \end{cases}
  \]
Preslikava $f$ ima samo dve periodični točki. To sta točki $a$ in $b$. Obe pa imata periodo 2. Ker nobena točka prostora $X$ ni negibna točka za preslikavo $f$, prostor $X$ ni prostor Šarkovskega.
\end{proof}
...
\begin{definicija}
Topološki prostor $X$ je \emph{s potmi povezan}, če za poljubni točki $a, b \in X$ obstaja zvezna preslikava $\gamma \colon [0, 1] \to X$, za katero je $\gamma(0) = a$ in $\gamma(1) = b$. Preslikavi $\gamma$ rečemo tudi pot med točkama $a$ in $b$.
\end{definicija}
...
\begin{definicija}
Naj bo $C$ graf funkcije $x \mapsto \sin\left(\frac{\pi}{x}\right)$ na intervalu $x \in (0 , 1]$ in naj bo $A$ daljica $\{ 0 \} \times [-1 , 1]$. \emph{Varšavski lok} je prostor homeomorfen prostoru $X = C \cup A$.
\end{definicija}
...
Pokažimo, da Varšavski lok ni s potmi povezan. Predpostavimo, da obstaja pot $p$ med točkama $(0, 0) \in A$ in $(1, 0) \in C$, za katero velja $p(0) = (0, 0)$ in $p(1) = (1, 0)$. Naj bo $x \colon \R^2 \to \R$ projekcija na os $x$. Funkcija $x$ je zvezna. Pot med točkama $(0, 0)$ in $(1, 0)$ se začne na množici $A$ in mora v neki točki ,,skočiti'' na množico $C$, ki je del Varšavskega loka s pozitivno koordinato $x$.   
Definiramo natančen čas, ko se to zgodi kot število
...
\begin{definicija}
Prostor $X$ je lokalno povezan prostor, če za vsako točko $x \in X$ in vsako odprto množico $U \subseteq X$, ki vsebuje točko $x$, obstaja taka odprta povezana množica $V \subseteq X$, da je $x \in V \subseteq U$. Prostoru, ki ni lokalno povezan pravimo lokalno nepovezan prostor.
\end{definicija}
...
Obstajajo pa tudi prostori, ki so povezani, vendar so lokalno nepovezani. Prepričajmo se, da temu pogoju ustreza Varšavski lok. Slika~\ref{fig:vl-lok-nepovezan} prikazuje na desni strani Varšavski lok, na levi strani pa je povečana okolica točke $(0, 0)$. Opazimo, da je vsaka okolica točke $(0, 0)$ vsebovana v levi okolici nepovezana, saj vsebuje nepovezane dele Varšavskega loka, ki jih lahko vidimo v povečani okolici na sliki~\ref{fig:vl-lok-nepovezan}.
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=1.1]{vk-shark.pdf}
  \caption[Varšavska krožnica]{Skica dokaza zveznosti funkcije $\widehat{f}$ iz trditve~\ref{trd:vk je prostor š}.}
  \label{fig:varšavski}
\end{figure}
